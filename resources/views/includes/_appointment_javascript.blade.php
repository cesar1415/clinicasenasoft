...
<script>
  ...
  @if (!isset($appointment))
    ...
    specialty.change(function () {
      $.ajax({
          ...
          success: function (data) {
            enfermera.empty();
            enfermera.append('<option disabled selected>-- Selecciona un medico --</option>');
            $.each(data, function (index, element) {
                enfermera.append('<option value="' + element.id + '">' + element.name + '</option>')
            });

          }
      });
    });
    enfermera.change(function () {
      $.ajax({
          url: "{{ route('enfermera.disable_dates') }}",
          data: {
    ...
    date_picker.on('set', function () {
      time_picker.set('clear');
      $.ajax({
          url: "{{ route('enfermera.disable_times') }}",
          data: {
            enfermera: enfermera.val(),
