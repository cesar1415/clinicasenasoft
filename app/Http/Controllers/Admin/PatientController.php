...
class PatientController extends Controller
{
    ...
    public function appointments(User $user){
        if (auth()->user()->hasRole('Enfermera')) {
            $appointments = $user->appointments->where('enfermera_id', auth()->user()->id)->sortBy('date');
        } else {
    ...
    public function invoice(User $user){
        if (auth()->user()->hasRole('Enfermera')) {
            ...
            foreach ($user_invoices as $key => $invoice) {
                if ($invoice->meta('enfermera') == auth()->user()->id) {
                    $invoices[] = $invoice;
                }
            }
    ...
    public function store_appointment(Request $request){
        ...
        $appointment = Appointment::create([
            'date' => $date->toDateTimeString(),
            'enfermera_id' => $request->enfermera,
            'status'=> 'pending',
